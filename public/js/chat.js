...
var VideoChat = {
  ...
  requestMediaStream: function (event) {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        localVideoText.text("Bougez moi");
  ...
  onMediaStream: function (stream) {
    ...
    Snackbar.show({
      text: "Merci de patienter, le medecin arrive",
      actionText: "D'accord",
      width: "750px",
      pos: "top-center",
  ...
  onCandidate: function (candidate) {
    // Update caption text
    captionText.text("Connexion en cours...");
...
function chatRoomFull() {
  alert(
    "Merci de verifier si vous n'avez pas plusiers onglets ouvert en même temps, car il apparait que vous êtes déjà en ligne."
  );
...
function pauseVideo() {
  ...
  if (videoIsPaused) {
    localVideoText.text("La vidéo est en pause");
    localVideoText.show();
    videoButtonIcon.classList.remove("fa-video");
    videoButtonIcon.classList.add("fa-video-slash");
    videoButtonText.innerText = "Mettre la vidéo en pause";
  } else {
    localVideoText.text("La Vidéo à repris");
    ...
    videoButtonText.innerText = "Remettre la vidéo";
  }
}
// End pause Video
...
function swap() {
  ...
  if (!VideoChat.connected) {
    alert("L'appel doit commencer avant de partager l'écran.");
    return;
  }
  ...
  if (mode === "camera") {
    ...
    Snackbar.show({
      text:
        "Choissisez ce que vous voulez partager et cliquez sur partager.",
      width: "400px",
      pos: "bottom-center",
      actionTextColor: "#616161",
      duration: 50000,
  ...
  if (
    ...
    if (document.pictureInPictureElement) {
      ...
      remoteVideoVanilla.requestPictureInPicture().catch((error) => {
        alert(
          "L'appel vidéo doit d'abord commencer avant d'utiliser cette fonctionnalité."
        );
      });
    }
  } else {
    alert(
      "Cette fonctionnalité n'est pas disponible dans votre navigateur."
    );
  }
}
//Picture in picture
...
function startUp() {
  ...
  // Set tab title
  document.title = "Rakiss - " + url.substring(url.lastIndexOf("/") + 1);
  ...
  Snackbar.show({
    text: "Merci d'autoriser la webcam et le micro",
    actionText: "Montrez moi comment faire",
    width: "455px",
    pos: "top-right",
    actionTextColor: "#616161",
    duration: 50000,
  ...
  captionText.text("Si rien ne se passe, merci de vérifier l'exactitude du code.").fadeIn();
